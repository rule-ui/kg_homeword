...
## 连接图形库，配置neo4j
## 更改成为你的neo4j的用户名和密码
graph = Graph("http://localhost:7474//browser/",auth = ('neo4j','xinfox666'),name="neo4j")
# 清空全部数据
graph.delete_all()
# 开启一个新的事务
graph.begin()
'''
with open("./data/data.json", 'r', encoding='utf-8') as input:
    data = json.load(input)
    num = 0
    for title, knowledge in data.items():
        num += 1
        print(title)
        knowledge['title'] = title
        node1 = Node('title', name=title, **knowledge)
        graph.merge(node1, 'title', 'name')
        knowledge.pop('title')
        for attr, content in knowledge.items():
            node2 = Node(attr, name=content)
            graph.merge(node2, attr, 'name')
            ## 创建关系
            rel = Relationship(node1, attr,node2)
            graph.merge(rel)
        if num == 300:
            break
'''
...
## csv源数据读取
storageData = pd.read_csv('./data/movies.csv',encoding = 'utf-8')
# 获取所有列标签
